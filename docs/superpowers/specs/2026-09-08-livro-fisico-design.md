...
- Modelo `claude-opus-5`; `client.messages.parse` com `output_config.format = zodOutputFormat(Esquema)` e `output_config.effort = "medium"`; `max_tokens: 16000` (2048 cortava a transcrição de uma página inteira e a resposta voltava vazia). Sem reserva de queda (fallback) para outro modelo: a tarefa não tem risco de recusa, e um `stop_reason = "refusal"` ou `parsed_output = null` é tratado como "a IA não conseguiu localizar", com os caminhos alternativos da seção 6.1; `stop_reason = "max_tokens"` tem mensagem própria ("A IA não terminou a resposta (limite de tamanho)…").
